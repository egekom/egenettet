...
class CMISRepositoryWrapper {
	...
	function __construct($url,$username=null,$password=null,$options=null,$headerpass=null) {
  $this->connect($url,$username,$password,$options, $headerpass);
	}
	...
	function connect($url,$username,$password,$options,$headerpass) {
	...
	function doRequest($url,$method="GET",$content=null,$contentType=null,$charset=null) {
		...
		if ($contentType) {
			...
			if ($this->headerpass) {
				$headers['X-Alfresco-Remote-User'] = $GLOBALS['user']->name;
				$headers['X-Alfresco-Run-As-Password'] = $this->headerpass;
			}
   ...
   if ($this->headerpass) {
    ...
    $object_node = $xmlnode->getElementsByTagName("object")->item(0);

    $prop_nodes = array();
    if ($object_node) {
      $prop_nodes = $object_node->getElementsByTagName("properties")->item(0)->childNodes;
    }
    ...
    if ($object_node) {
      $renditions = $object_node->getElementsByTagName("rendition");
      ...
      if($renditions->length > 0){
		...
		if ($object_node) {
      $properties = $object_node->getElementsByTagName("properties")->item(0);
...
<?php
    ...
    // ProPeople patch to get numItems from query result.
    $objs->numItems = 0;
    if (!empty($ret->body)) {
      $doc=new DOMDocument();
      $doc->loadXML($ret->body);
      $cmisRaNumItems = CMISRepositoryWrapper::doXQueryFromNode($doc, "//atom:feed/cmisra:numItems");
      if ($cmisRaNumItems && $cmisRaNumItems->length > 0) {
        $objs->numItems = $cmisRaNumItems->item(0)->nodeValue;
      }
      else {
        $objs->numItems = 0;
      }
