    ...
    attach: function (context) {
      $('.homebox-portlet:not(.homebox-processed)', context).addClass('homebox-processed').each(function () {
        ...
        $portletConfig.find('.form-submit').click(function () {
          ...
          Drupal.homebox.config[$portlet.attr('id')] = tempclass;
          console.log('saving');
        });

        // Attach click event on close
        $portletHeader.find('.portlet-close').click(function () {
          $confirmationBox = $portletHeader.find('.confirmation-box');
          $confirmationBox.show();
        });

        // Attach click event on confirmation box options
        $portletHeader.find('.confirm').click(function () {
          $portlet.hide();
          Drupal.homebox.equalizeColumnsHeights();
          Drupal.homebox.pageChanged();
        });
        $portletHeader.find('.refuse').click(function () {
          $portletHeader.find('.confirmation-box').hide();
        });
